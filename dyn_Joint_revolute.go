...
func (joint *RevoluteJoint) InitVelocityConstraints(data SolverData) {
	...
	if data.Step.WarmStarting {
		...
		joint.impulse.OperatorScalarMulInplace(data.Step.DtRatio)
		...
		wA -= iA * (Vec2Cross(joint.M_rA, P) + axialImpulse)

		vB.OperatorPlusInplace(Vec2MulScalar(mB, P))
		wB += iB * (Vec2Cross(joint.M_rB, P) + axialImpulse)
	} else {
...
func (joint *RevoluteJoint) SolvePositionConstraints(data SolverData) bool {
	...
	if joint.M_enableLimit && fixedRotation == false {
		...
		if math.Abs(joint.M_upperAngle-joint.M_lowerAngle) < 2.0*angularSlop {
			...
			C = FloatClamp(angle-joint.M_lowerAngle+angularSlop, -maxAngularCorrection, 0.0)
		} else if angle >= joint.M_upperAngle {
			// Prevent large angular corrections and allow some slop.
			C = FloatClamp(angle-joint.M_upperAngle-angularSlop, 0.0, maxAngularCorrection)
		}
...
func (joint RevoluteJoint) GetJointAngle() float64 {
	bA := joint.bodyA
	bB := joint.bodyB
	return bB.sweep.A - bA.sweep.A - joint.M_referenceAngle
}
